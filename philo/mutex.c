...
/*   Updated: 2022/06/16 20:20:12 by vfuhlenb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_left_fork(t_philo *philo)
{
	...
	{
		pthread_mutex_lock(&philo->mutex[philo->data->nbr_philo - 1]);
		if (philo->fork[philo->data->nbr_philo - 1])
			status = 1;
		pthread_mutex_unlock(&philo->mutex[philo->data->nbr_philo - 1]);
	}
	return (status);
}
...
void	handle_left_fork(t_philo *philo, bool action)
{
	...
	{
		pthread_mutex_lock(&philo->mutex[philo->data->nbr_philo - 1]);
		philo->fork[philo->data->nbr_philo - 1] = action;
		pthread_mutex_unlock(&philo->mutex[philo->data->nbr_philo - 1]);
	}
}
